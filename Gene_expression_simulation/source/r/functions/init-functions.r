#############################################################################################################
# 
# This is a script used to store all functions used in this project
# This should be sourced by the main init script after the working directory
# has been set
#
# Program:  init-functions.r
# Version:  RSEQREP 2.3.0
# Author:   zshahjahan
# Purpose:  Initializes functions.
# Input:    
# Output:  	N/A
#############################################################################################################

#TODO: create separate functions scripts such that there exists
#a function script for plotting
#a function script for pre-processing
#a function script for processing the analysis steps after
#fitting the linear models

#function to check and install required packages
check_and_install.func = function(package_name) {
  installed_packages = character(length(package_name))
  installed_status = logical(length(package_name))
  
  from_bioc = BiocManager::valid(package_name)
  
  if (require(package_name, character.only = TRUE)) {
    installed_status = c(installed_status, TRUE)
    ...
    installed_packages = c(installed_packages, package_name)
      ...
      #TODO: Add code to install bioconductor package
      BiocManager::install(package_name,dependencies=TRUE,ask = FALSE)
    }
  }
  
  # Print the results
  cat("Package(s) already installed:\n")
  cat(installed_packages[installed_status], "\n")
...
#Function to get lower triangle of correlation matrix; helper function for the
#get correlation heatmap function
get_lower_tri.func = function(cormat){
  cormat[upper.tri(cormat)] = NA
  return(cormat)
}

reorder_cormat.func = function(cormat){
...
#Function that returns a correlation heatmap
get_correl_heatmap.func = function(mat,
                                   method=c("pearson","kendall","spearman"),
                                   sig.dig=3) {
  dependenci.lib = c("reshape2","ggplot2")
  
  sapply(dependenci.lib, require, character.only=T)
  
  cormat.gene=signif(cor(mat,method=method),sig.dig)
  reordered.cormat = reorder_cormat.func(cormat.gene)
  lower_tri.mat = get_lower_tri.func(reordered.cormat)
  melt.cormat = reshape2::melt(lower_tri.mat,na.rm=T)
  
  #Create heatmap
  # Create a ggheatmap
  ggheatmap = ggplot(melt.cormat, aes(Var2, Var1, fill = value))+
    ...
    theme(
      ...
      legend.position = "none",#TODO: Create legend conditionally
...
#Function to compute geometric mean titers
geo_mean.func = function(x){
  y=exp(mean(log(x)))
  return(y)
}

#Got this  heatmap function that can be used to color the rows and columns
#Takes in a matrix of values instead of just a vector
...
#Apply function for rolling summary statistics
rollapply = function(x,n,f, ...){
  #Output of the same length as input
  out = rep(NA,length(x))
  #Offset should be an integer
  offset = trunc(n/2)
  #Inside the for-loop perform rolling computaions
  #go from offset+1 to end of x, from this subtract the window and add back
  #offset+1
